...
function Dropdown({placeholder, options, value, onChange}) {
    ...
    return (
        <div className="relative">
            ...
            {open && (
                <ul className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-52 overflow-y-auto">
                    {options.map((opt) => (
                        <li key={opt}>
                            <button
                                type="button"
        ...
        <div className="min-h-screen bg-background">
            ...
            <div className="relative z-10 max-w-xl mx-auto px-4" style={{marginTop: '-160px'}}>
                <div className="bg-white rounded-2xl shadow-2xl p-6">
                    ...
                    <div className="flex rounded-xl border border-gray-200 mb-6 overflow-hidden">
                        <button
                            className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                                tab === 'service' ? 'bg-primary text-primary-foreground' : 'text-gray-500 bg-white hover:bg-gray-50'
                            }`}
                            onClick={() => {
                                setTab('service');
                        ...
                        <button
                            className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                                tab === 'doctor' ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'text-gray-500 bg-white hover:bg-gray-50'
                            }`}
                            onClick={() => {
                                setTab('doctor');
                                setHasSearched(false);
                    ...
                    {tab === 'service' ? (
                        ...
                        <div className="mb-4">
                            ...
                            <div className="relative">
                                ...
                                {(searchLoading || searchError || hasSearched) && doctorName.length >= 2 && (
                                    <div
                                        className="absolute left-0 right-0 mt-2 bg-white border border-gray-200 rounded-2xl shadow-lg z-20">
                                        {searchLoading && (
                                        ...
                                        {!searchLoading && !searchError && doctorResults.length === 0 && (
                                            <div className="p-4 text-center text-gray-500">هیچ پزشکی با این نام پیدا
                                                نشد.</div>
                                        )}
                                        {!searchLoading && !searchError && doctorResults.length > 0 && (
                                            <ul className="divide-y divide-gray-100">
                                                {doctorResults.map((doctor) => (
                                                    <li key={doctor.id} className="py-3 px-4 hover:bg-gray-50">
                                                        ...
                                                        >
                                                            <div
                                                                className="font-medium text-gray-800">{doctor.user_full_name || 'دکتر'}</div>
                                                            <div
                                                                className="text-sm text-gray-600">{doctor.specialty_name || 'تخصص نامشخص'}</div>
                                                        </button>
                                                    </li>
                                                ))}
                                            </ul>
                    ...
                    >
                        <ChevronDown size={16} className={`transition-transform ${moreFilters ? 'rotate-180' : ''}`}/>
                        فیلترهای بیشتر
                    </button>
            ...
            {/* ── نتایج سرچ تب service ── */}
            {hasSearched && tab === 'service' && (
                <div className="relative z-10 max-w-xl mx-auto px-4 mt-4">
                    <div className="bg-white rounded-2xl shadow-xl p-4">
                        {searchLoading && (
                            <div className="text-center text-gray-500 py-4">در حال جستجو...</div>
                        )}
                        {searchError && (
                            <div className="text-center text-red-500 py-4">خطا در جستجو</div>
                        )}
                        {!searchLoading && !searchError && doctorResults.length === 0 && (
                            <div className="text-center text-gray-500 py-4">هیچ پزشکی پیدا نشد.</div>
                        )}
                        {!searchLoading && !searchError && doctorResults.length > 0 && (
                            <ul className="divide-y divide-gray-100">
                                {doctorResults.map((doctor) => (
                                    <li key={doctor.id} className="py-3 px-2 hover:bg-gray-50 rounded-xl">
                                        <div className="font-semibold text-gray-800">
                                            دکتر {doctor.user_full_name || '—'}
                                        </div>
                                        <div className="text-sm text-gray-500 mt-0.5">
                                            {doctor.specialty_name}{doctor.city ? ` · ${doctor.city}` : ''}
                                        </div>
                                        <div className="text-sm text-primary mt-0.5">
                                            {doctor.consultation_fee
                                                ? `${parseInt(doctor.consultation_fee).toLocaleString('fa')} تومان`
                                                : ''}
                                        </div>
                                    </li>
                                ))}
            ...
            <section className="max-w-5xl mx-auto px-4 pt-24 pb-12">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <button
                        className="flex items-center gap-1 text-[var(--primary)] text-sm font-medium hover:underline">
                        <ChevronLeft size={16}/>
                        مشاهده همه
                    </button>
                </div>
